...
def read(last_msg,sync_first=False):
    global ser
    global isWrite
    global live
        ...
        if(s==b''):
            if(not once and not sync_first):
                ...
                if (time()-last_check_time>=1.0):
                    ser.write(struct.pack(">6s",b"#stts&"))
                    print("send")
    ...
    last_check_time=time()
    # try:
    #     if (struct.unpack(">3s",out_msg)==b'oks') and not sync_first:
    #         send(last_msg)
    #         return read(last_msg=last_msg)
    # except:
    #     pass
    return out_msg
...
def handle_board_led(req):
    ...
    if(leds!=sost_board_led):
        ...
        if(j==len(leds)):
            ...
            send(msg)
            otv=struct.unpack(">4s",read(last_msg=msg))[0]
            send_log("response: "+ str(otv, encoding='utf-8'))
            aled=True
        else:
            aled=False
...
while ((tmp!=b"oks") and (not rospy.is_shutdown())):
    ...
    send(msg)
    tmp=struct.unpack(">3s",read(last_msg=msg,sync_first=True))[0]
sleep(1)
send(struct.pack(">6s",b"#bnum&"))
_,num=struct.unpack(">4sB",read(last_msg=struct.pack(">6s",b"#bnum&")))
...
while not rospy.is_shutdown():
    if (time()-last_check_time>=1.0) and first:
        msg=struct.pack(">6s",b"#stts&")
        send(msg)
